<template>
    <div>
                            ...
                            <li v-for="item in cartList">
                                <div class="cart-tab-1">
                                    <div class="cart-item-check">
                                        <a href="javascript:;" @click="addCartNum('selected',item)" class="checkbox-btn item-check-btn" :class="{'check':item.checked>=1?true:false}">
                                    ...
                                    <div class="item-quantity">
                                        <div class="select-self select-self-open">
                                            <div class="select-self-area">
                                                <a class="input-sub" @click="addCartNum('jian',item)">-</a>
                                                <span class="select-ipt" v-text="item.productNum"></span>
                                                <a class="input-add" @click="addCartNum('add',item)">+</a>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                                ...
                                <div class="cart-tab-5">
                                    <div class="cart-item-opration" @click="delGoods(item.productId)">
                      ...
                      <svg class="icon icon-ok"><use xlink:href="#icon-ok"/></svg>
                        ...
                        <div class="cart-foot-r">
                            ...
                            <div class="btn-wrap" @click="checkout">
                                <a class="btn btn--red" style="padding: 0 25px">支付</a>
                            </div>
                        </div>
                    </div>
                </div>
        ...
        <Model :mdShow="showDelModel">
            <p slot="title">删除</p>
            <div slot="message">你确定删除此条数据吗？</div>
            <div slot="btnGroup">
                <a class="btn-login" href="javascript:;" @click="delcartgoods">确定</a>
                <a class="btn-login" href="javascript:;" @click="hideModel">取消</a>
            </div>
        </Model>
    </div>
</template>
...
<script>
    ...
    import {currency} from  './../util/currfomart'
    export default{
        components: {
            NavHeader,
            NavFooter,
        ...
        methods : {
            init () {
                axios.get("/cartList").then((respone) => {
                    let res = respone.data;
                    if (res.status == '0') {
                        this.cartList = res.result;
                    } else {
                        this.$layer.msg("你还未登录");
                    }
                }).catch((err) => {
                    alert("获取商品列表失败" + err);
                });
            ...
            delcartgoods () {
                ...
                axios.get('/delgoods',{
                    ...
                    if(responens.data.status=='0') {
                        this.$layer.msg("商品删除成功");
                        this.hideModel();
                        this.init();
                    }else{
                        this.$layer.msg("network bad 商品删除失败");
                        this.hideModel();
                    }
                }).catch((err)=>{
                    this.$layer.msg("错误"+err);
                })
            },
            addCartNum (type,item) {
                if(type=='add'){
                    item.productNum++
                }else if (type=='jian') {
            ...
            toggleAll () {
               ...
               axios.post('/selectAll',{
                   checked:this.checkAll==true?1:0
                }).then((data) => {
