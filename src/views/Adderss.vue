<template>
    <div class="addersspage">
        <nav-header></nav-header>
        <nav-bread>
            <span slot="listA">收货地址</span>
        </nav-bread>
            ...
            <div class="container">
                <div class="checkout-addr">
                    ...
                    <div class="addr-list-wrap">
                        <div class="addr-list">
                            <ul>
                                <li v-for="(item,index) in adderssdataFilg" :class="{'check':checkIndex==index}" @click="checkAdders(index)">
                                    <dl>
                                        <dt>{{item.userName}}</dt>
                                        <dd class="address">{{item.streetName}}</dd>
                                        <dd class="tel">{{item.tel}}</dd>
                                    </dl>
                                    <div class="addr-opration addr-del" @click="deladders(item.addressId)">
                        ...
                        <div class="shipping-addr-more" @click="adderssShowMore" :class="{'open':limit>3}">
                    ...
                    <div class="next-btn-wrap" @click="next">
                        <a class="btn btn--m btn--red" style="padding: 0">下一步</a>
                    </div>
                </div>
            </div>
        </div>
...
<script>
    ...
    export default {
        ...
        computed:{
            adderssdataFilg () {
                return this.adderssdata.slice(0,this.limit-1)
            }
        },
        methods: {
            init () {
                axios.get('/getAdderss').then((resopene)=>{
                    let res = resopene.data;
                    this.adderssdata = res.result;
                })
            },
            adderssShowMore () {
                if(this.limit==3) {
                    this.limit = this.adderssdata.length+1;
                }else{
                    this.limit = 3;
                }
            },
